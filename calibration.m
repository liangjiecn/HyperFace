...
results = sface(same,:);
tsubject = strcmp(results(:,1),sface(:,1));
rsubject = sum(tsubject)/length(sface)

tdate = strcmp(results(:,2),sface(:,2));
rdate = sum(tdate)/length(sface)

ttotal= or(tsubject, tdate);
rtotal = sum(ttotal)/length(sface)
