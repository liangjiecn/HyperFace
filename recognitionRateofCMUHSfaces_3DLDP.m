...
faceDataName = fullfile(pathstr, 'temp', [name, 'temp.mat']);
...
for indexofSubject = 1:47
    ...
    while(indexofstruct <= numofstruct) 
         ...
         face = smooth3(face, 'gaussian', [9 9 9], 0.8);
         face = uint8(face);
%          ngaborFace = uint8(ngaborFace*255);
            % extract 3D LDP features 
         [hist] = mex3DLDP(face, subImageSize, order);         
         faceData{indexofFace,1} = str2double(subjectSet(indexofSubject).name);  
%          faceData{indexofFace,2} = hist(1:xynumBins)';  
         faceData{indexofFace,2} = hist'; 
